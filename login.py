...
class LogInPage:
    def __init__(self ,root, username_lg = ""):
        ...
        self.frame = Frame(self.root, width=530, height=580, bg='black')

        self.frame.place(x=489, y=160)
        self.heading = Label(self.frame, text='User Login', fg='Blue',bg='black', font=('copper black', 28, 'bold'))
        self.heading.place(x=160, y=100)
        self.img1 = PhotoImage(file='images/gamehouse2.png')

        self.l = Label(self.frame, text = "Home",fg= 'white', bg='black',  font =('Century', 9,'bold'))
        ...
        Frame(self.frame, width=295, height=2, bg='black').place(x=110, y=235)

        # -------------------------------------------------------------------------------------------
        def on_enter(e):
            self.user1.delete(0, 'end')
            self.user1.config(show = "*")
        def on_leave(e):
            name = self.user1.get()
            if name == '':
                self.user1.config(show = "")
        ...
        Frame(self.frame, width=295, height=2, bg='black').place(x=110, y=320)
        ...
        def login( ):
            ...
            if row is not None:
                ...
                messagebox.askretrycancel ( "Invalid" , "Username is not registered!" )
        ...
        self.label = Label(self.frame, text="Don't have an Account?", fg='red', bg='black', font=('Microsoft YaHei UI Light', 12, 'bold'))
        self.label.place(x=80, y=460)
        self.button2 = Button(self.frame, width=15, text='Create an Account', bg='black', font=('Arial Black', 10,), border=0, cursor='hand2', fg='White', command=newAcc)
        self.button2.place(x=300, y=459)
        self.button3 = Button(self.frame, width=15, text='Forgot Password ?', bg='black', font=('Century', 12,), border=0, cursor='hand2', fg='White', command=forgot_password)
        self.button3.place(x=300, y=350)
        self.b = Button ( self.frame , width = 50 , height = 50 , bg = 'black' , border = 0 , image = self.img1 ,
                          command = home )
